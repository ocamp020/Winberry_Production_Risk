...
function [A_ss,K_ss,N_ss,Y_ss,Earnings_W,Earnings_E] = Graphs_Tables(model,r,w,mDBN_W,mDBN_E,mAp_W,mAp_E,OC_W,OC_E1,OC_E2,V_W,V_E,Transition)

global mmu ddelta llambda tau_k tau_n AA aalpha...
       n_A n_E n_Z vA_Grid vE_Grid mA_Grid_E mA_Grid_W mZ_Grid mE_Grid
...
%% Capital and Profits and Earnings
    ...
    Earnings_E   = r*(1-tau_k)*mA_Grid_E + (1-tau_k)*Pr ;
    Earnings_E_vec = Earnings_E(:); 
    [Earnings_E_vec,Earnings_E_ind] = sort(Earnings_E_vec) ;
    % Labor Earnings
    Earnings_W   = r*(1-tau_k)*mA_Grid_W(:,2:n_E,:) + w*mE_Grid(:,2:n_E,:) ; 
...
%% Distribution and Composition
    figure; 
    plot(vA_Grid,[sum(sum(mDBN_W,3),2)/sum(sum(sum(mDBN_W))),sum(sum(mDBN_E,3),2)/sum(sum(sum(mDBN_E))),sum(sum(mDBN_W,3),2)+sum(sum(mDBN_E,3),2)],'-')
    xlabel('Wealth')
...
%% Top Wealth Shares
    ...
    DBN_A   = sum(sum(mDBN_W,3),2)+sum(sum(mDBN_E,3),2) ;
...
%% Aggregates
    ...
    A_ss = vA_Grid'*(sum(sum(mDBN_W,3),2)+sum(sum(mDBN_E,3),2)) ;
...
%% Policy and Value Functions
    ...
    % Saving Functions
        ...
        i_plot  = 1;
        for i_z = 1:n_Z
            subplot(1,n_Z,i_plot); hold on; 
            plot(vA_Grid,vA_Grid(mAp_E(:,1,i_z)),'linewidth',2)
